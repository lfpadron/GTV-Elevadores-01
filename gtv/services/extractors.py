...
def extract_document_structured(
    ...
    *,
    ...
    if document_type == "reporte":
        ...
        estimate_equipment_ok = bool(common["equipment_text"]) and bool(
            common["tower"] or "elev" in normalize_for_match(common["equipment_text"] or "")
        )
        status = _resolve_status(
            {
...
def _extract_estimate(
    ...
    items = _extract_estimate_items(layout_lines, raw_lines)
...
def _extract_estimate_items(layout_lines: list[str], raw_lines: list[str]) -> list[dict]:
    items: list[dict] = []
...
def _extract_position_from_equipment(equipment_text: str | None) -> str | None:
    ...
    for word in ("izquierdo", "derecho", "carga", "unico"):
